...
-- =============================================================================
-- Integrate Limpeza synchronization into Escala's publish flow
-- (product decision 2026-09-25: explicit sync on publish is the primary
-- Limpeza architecture — see 20260925_102's header comment).
--
-- This redefines escala_processar_importacao (unchanged in every other
-- respect from 20260914_009_block_zero_diff_revision_publish.sql — that and
-- every earlier Escala migration remain immutable history) to call
-- limpeza_sincronizar_datas_afetadas exactly once, only when a publish
-- actually wrote a new escala_publicacoes row (v_status = 'publicado'):
--
--   - Revision of an already-published month (is_revisao = true): syncs only
--     the distinct dates that appear in v_diff — the rows the diff engine
--     already determined changed status/hours between the old and new
--     escala_entradas. Every other future date's Limpeza rows are left
--     completely untouched, so one changed shift never regenerates a whole
--     month.
--   - First-time publish of a month (is_revisao = false): v_diff is always
--     empty by construction (nothing to diff against), so instead every day
...
begin
  ...
  if p_publicar and not (v_is_revisao and jsonb_array_length(v_diff) = 0) then
    ...
    -- Limpeza sync hook (2026-09-25). Revision: only the dates the diff
    -- engine flagged as changed. First-time publish: the whole month, since
    -- there is nothing to diff against yet. See the header comment above for
    -- the full rationale, including why this is wrapped in its own
    -- exception guard.
    begin
