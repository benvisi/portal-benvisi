...
-- =============================================================================
-- Fix: escala_processar_importacao's temp tables used `on commit drop` only,
-- which is dropped at transaction end — not between calls within the same
-- transaction. Two invocations in one transaction (confirmed while manually
-- verifying this RPC via execute_sql, which runs a whole query as one
-- transaction) raised "relation already exists". PostgREST always runs one
-- RPC call per transaction in normal Portal usage, so this never surfaced in
-- the browser flow, but it's a latent fragility worth closing regardless.
-- Fix: explicitly drop both temp tables first, so the function is safe to
-- call more than once per transaction. Everything else is unchanged from
-- 20260914_004.
-- =============================================================================
