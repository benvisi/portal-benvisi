...
-- =============================================================================
-- Escala Admin upload — refuse to publish a zero-diff revision (QA round 2)
--
-- When the selected month already has an active publication and the
-- normalized upload is identical to it (is_revisao = true, diff = []),
-- publishing would only create a redundant version with nothing new. This
-- is now refused authoritatively here — even if p_publicar = true is
-- forced, the write is skipped and v_status stays 'pronto' — so the guard
-- holds regardless of what the client does or doesn't disable. The client
-- (src/lib/escalaImportacaoEstado.ts) mirrors this same is_revisao/diff
-- check to hide the "Publicar escala" button before ever attempting the
-- call; this is the authoritative enforcement behind that UI behavior.
--
-- First-time publication of a month with no active publication (is_revisao
-- = false) is entirely unaffected — diff is always empty then anyway, but
-- the guard only ever applies when is_revisao is true.
-- =============================================================================
