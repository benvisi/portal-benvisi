...
**Implementation status (Milestone 4E — Inventory Backend V1):**
...
- **`estoque_cores_mapeamento`** — curated colour dictionary, key
  ...
  ad hoc import). `cor_descricao_linx` is internal Linx source metadata used
...
- `buscar_produtos_estoque(session, termo)` — primary: case-insensitive
  **prefix** match on `produto` (`PH4` → `PH4012`, `PH4014`, …, never
  `PH5522`); secondary: `desc_produto` contains. One row per `produto`. No
...
**Links Importantes**, **Mensagens para WhatsApp**, **Escala V1**, and **Contagem de Embalagens** (V1 plus the 4D.1 resumable-draft update) are implemented and QA-passed — see Epic 4, Milestones 4A, 4B, 4C, 4D, and 4D.1 (section 16.2) for their full scope. Every other workflow below remains unimplemented future/planned scope, distinct from the fast read-only Consulta de Estoque experience (section 9). Sequencing/priority among them is tracked in section 16.2's Epic 4 near-term sequence, not here.
...
**Contagem de Embalagens — IMPLEMENTED / QA COMPLETE (Milestones 4D and 4D.1, section 16.2)**
...
- **4D.1 — resumable draft (full detail in the Milestone 4D.1 section below).** The counting form is no longer single-shot: only one packaging count can be in progress store-wide at a time, any authenticated employee can open the form and resume it (with a "Contagem iniciada por {nome} em {data/hora}" banner showing who started it), a **Salvar** button explicitly persists progress at any point (autosave was tried and dropped as unreliable), and a **Cancelar contagem** confirm action abandons an in-progress draft outright. **Finalizar contagem** (the renamed submit action) is otherwise unchanged — same completeness validation, same `pendente_revisao` handoff to Admin review;
...
- **not** in V1/4D.1 (future): Reposição Inteligente / purchasing recommendations; email notification on submit (Admin pending-review UI is the notification mechanism); recurrence / monthly required tasks / reminders / overdue logic; management correction/reopen of an already-finalized count (see the 4D.1 FUTURE note below).
...
- Level 2 (navigation tiles inside a section hub, e.g. Operações, Conhecimento & Cultura) uses a muted mid-tone green (`#416352`), with white text/icons. Measured contrast against white is ~6.71:1, comfortably clearing the formal WCAG AA text threshold (4.5:1) — no legibility caveat applies;
- Level 3 is reserved for a still-deeper future navigation tier — no such tier exists in the app today. It is an independent, pale sage-gray (`#CFD8D3`), not reusing the table zebra-stripe color, paired with dark (not white) text/icons, since that background is light. No Level 4 exists yet; white would be the natural next step in this progression if a fourth tier is ever needed;
...
4. Operações — Escala V1 (Milestone 4C, section 10) — **complete, QA passed** (4C.1 foundation + 4C.2/4C.3 employee-facing Dia/Semana/Mês UI + 4C.4 final September 2026 publication + 4C.5 Admin upload/publish workflow — the real Excel/`.xlsx` publish/import flow, no longer future scope);
...
4C.1 (foundation) + 4C.2 (employee-facing Dia / Semana / Mês UI) + 4C.3 (provisional data import, Gestão model change, QA revisions + polish) + 4C.4 (final September 2026 publication) + 4C.5 (Admin `.xlsx` upload/publish workflow, section 4C.5 below) are all implemented and applied. Product-owner browser QA is complete and **PASS** — covering the 4C.2 rounds (normal weekday, Sunday/single-shift, Intermediário, Folga/Férias, Gestão visibility), the 4C.3 revisions (colour-coded sections, section-hours-in-heading, section reorder, Gestão bucketing + withheld manager hours, `Dia` rename, Mês→Dia navigation, apelido-first identity, shared `AuthUtilityBar`, `/operacoes` tile reorder), the 4C.4 final-workbook publication (Dia / Semana / Mês), and the full 4C.5 Admin upload/publish round (see that section for the detailed QA record). The September 2026 schedule has since been revised once more through the 4C.5 Admin uploader (a legitimate Favacho shift correction on 18/09) and is at `v3` as the active publication; `v1`/`v2` are retained as inactive history.
...
- ~~the real Excel → structured-data UUID / technical-column mapping and publish flow (V1.1)~~ — **DONE, Milestone 4C.5** (Admin `.xlsx` upload/preview/publish workflow, no UUID/technical column ever exposed in the workbook);
...
**V1.1 — Admin Escala spreadsheet uploader — DONE, see Milestone 4C.5 below.** The `Admin → Escala` upload/preview/publish workflow described here at concept stage has been implemented and QA-passed exactly along these lines: Excel/Google Sheets remains the sole authoring tool; `.xlsx` upload in Portal; server-side (SQL RPC) parsing/normalization/validation using the same V1 rules; preview with employee-identity resolution, unmapped/ambiguous-row blocking, and a diff against the currently active publication; atomic publish as a new monthly version with the previous one flipped to inactive history; and full support for mid-month republishing of a revised workbook through the same flow. See Milestone 4C.5 for the as-built architecture, validation matrix, and QA record.
...
### Milestone 4C.5 — Escala Admin Upload/Publish Workflow (V1.1)

**Status:** IMPLEMENTED / QA COMPLETE

Replaces the manual Supabase-MCP publish process used through 4C.4 with a real `Administrativo → Escala` workflow: upload the `.xlsx` schedule → Portal parses/validates/previews it → Admin confirms → publish. Excel/Google Sheets remains the sole schedule-authoring tool; Portal is publish/consumption only, exactly as scoped in the V1.1 concept above — nothing here turns Portal into a schedule editor.

**Architecture.** One authoritative SECURITY DEFINER RPC, `escala_processar_importacao` (`20260914_004`, fixed/extended by `20260914_005/006/008/009`), does everything: Administrador-only authorization, employee-identity resolution, value normalization, BLOCK/WARNING validation, diff-vs-active-publication, and (only when explicitly requested and nothing blocks) the atomic publish. The browser's only responsibility is *structural* `.xlsx` parsing (`src/lib/escalaXlsx.ts`, via `exceljs`) — finding weekly schedule blocks and employee rows and extracting raw `(nome_planilha, data, valor)` tuples; it never interprets FOLGA/FÉRIAS/hour-range/MANHÃ-TARDE business rules itself. The client calls this one RPC twice with an identical payload — once with `p_publicar = false` for the preview/diff screen, once with `true` to publish — so preview and publish can never drift apart, and server-side re-validation at publish time is automatic rather than a second implementation to keep in sync. No staging tables and no separate mapping table were introduced; both were considered and dropped as unnecessary for a few-hundred-row, human-triggered, per-request-transaction workflow (unlike the Estoque V2 sync's unattended, high-volume, claim/lock-requiring architecture — see Milestone 4F).

**Employee identity: `escala_nome_planilha`, not `apelido`.** The first implementation matched spreadsheet rows against `funcionarios.apelido` directly (it already was unique and matched the real workbook exactly). QA round 1 rejected this on architectural grounds: `apelido` is a display/nickname field, and coupling it to a machine-import identity would mean a future nickname change could silently break Escala imports. `funcionarios.escala_nome_planilha` (`citext`, unique when set, migration `20260914_007`) is now the dedicated, independently-editable canonical spreadsheet identity, backfilled once from each active employee's then-current `apelido` and verified against the real September 2026 workbook. Matching is exact and case-insensitive; no fuzzy matching. An employee with no `escala_nome_planilha` configured is simply never matchable — a workbook row for them with real schedule data blocks as unmapped (see validation matrix below), exactly like any other unmapped name.

**Month/sheet detection — explicit ambiguity handling (QA round 1 fix).** The real workbook (`ESCALA DE TRABALHO ... .xlsx`) carries every historical monthly tab (Fev 2025 → Out 2026), almost all hidden except the current month. A candidate sheet is any tab whose name exactly matches `<mês> <ano>` (abbreviated or full Portuguese month name, e.g. `set 2026` or `setembro 2026` — a suffixed variant like `mar 2026 - original` does not match) **and** contains a real weekly date-header structure (7 consecutive date-valued cells, one calendar day apart) — sheet visibility and how much data a sheet contains are never inputs to detection or to auto-selection. Auto-selection (`escolherCandidataPreselecionada`, `src/lib/escalaXlsx.ts`) only fires when there is exactly one valid candidate, or the uploaded file's name reliably names exactly one month/year that matches exactly one candidate (`parseFilenameMonth` — e.g. "...set 2026 final.xlsx" → Setembro/2026). Any other case — several valid monthly sheets and no reliable filename match — leaves the selection blank and requires the Admin to choose explicitly from a dropdown before anything is parsed; the selected month is always shown in a highlighted "Mês a publicar" callout.

**Validation philosophy — guide, don't police.** BLOCK only when Portal cannot safely interpret the data; WARNING for everything else understandable-but-worth-a-look.

| Condition | Severity | Notes |
|---|---|---|
| Unmapped employee identity with real schedule data | BLOCK | No fuzzy matching; fix `escala_nome_planilha` or the sheet, re-upload |
| Unmapped employee identity, entirely blank for the month | *(ignored — no block, no warning)* | e.g. a roster row like Monica with no mapping and no data |
| Duplicate `(funcionário, data)` after normalization | BLOCK | |
| Unrecognized cell value (not blank/FOLGA/FÉRIAS/`HH:MM-HH:MM`/MANHÃ/TARDE) | BLOCK | |
| Malformed/incoherent hour range (end ≤ start) | BLOCK | |
| MANHÃ/TARDE on a day the store is closed (cannot derive hours) | BLOCK | |
| Date outside the selected target month | BLOCK | Spillover columns are already excluded client-side; this is the server-side defense-in-depth check |
| Zero usable rows after normalization | BLOCK | |
| Active, mapped employee absent from the sheet entirely | WARNING | Overridable via "Publicar mesmo assim" |
| Mapped employee present in the sheet but blank all month | WARNING | |
| Revision with zero net changes vs. the active publication | **BLOCK — publish never offered** | See below; distinct from the other BLOCK cases in that it is never overridable |

**Extended-hours acceptance.** Explicit `HH:MM-HH:MM` ranges are validated only for clock-syntax coherence (`fim > início`), never against a hardcoded "ordinary mall hours" range — an occasional extended-hours day (e.g. `10:00-23:00`) is accepted exactly like any other explicit range. MANHÃ/TARDE derivation reuses the existing `loja_horario_do_dia` store-hours function (splitting that day's actual operating window at its midpoint) rather than any hardcoded fixed hours, so it naturally adapts to an exceptional extended-hours day the same way it already adapts to Sunday's shorter hours.

**Preview/diff behavior.** The preview call returns funcionário/dias/registros counts, the BLOCK/WARNING lists, and — whenever the target month already has an active publication — a diff classifying every changed `(funcionário, data)` as `adicionado` (blank → scheduled), `removido` (scheduled → blank), or `alterado` (status/hours changed), rendered as e.g. "Favacho — 18/09 / A confirmar → 16:00–22:00". Unchanged entries are never shown.

**Zero-diff publication protection (QA round 2 fix).** When the diff comes back empty for a month that already has an active publication, Portal refuses to create a redundant version: the UI shows "Esta escala já está publicada." / "Nenhuma alteração foi encontrada em relação à versão atual." with no publish action offered, and — as the authoritative enforcement, not merely a UI nicety — `escala_processar_importacao` itself refuses to write even if publish is forced (`p_publicar = true` with `is_revisao = true` and an empty diff is a no-op, returning `status: "pronto"`). First-time publication of a month with no active publication is entirely unaffected by this guard.

**Atomic publish + history.** Publishing deactivates the month's current active `escala_publicacoes` row and inserts the new one (with `nome_arquivo` and `publicacao_anterior_id` recorded) plus its `escala_entradas` inside one transaction — never a partial replacement. `get_escala_publicacoes_historico` (Administrador-only) lists every publication for a month with a computed version ordinal (`v1`, `v2`, ...), active/substituted state, publisher, source filename, and row count. No rollback UI — re-uploading the intended workbook is the accepted recovery path, per the approved V1 scope.

**Administrativo IA change.** `/administrativo` changed from a single settings page into a plain two-card module hub (`Escala`, `Atendimento`), matching the `/operacoes` hub pattern — mixing module cards with inline settings did not scale. The pre-existing Política do Checklist controls (Milestone 2C.1) moved unchanged, same components/hooks/server-side enforcement, to `/administrativo/atendimento`.

**Testing.** First automated test runner introduced into the repo (`vitest`), scoped deliberately to the deterministic, pure importer logic rather than a Portal-wide testing initiative: structural `.xlsx` parsing (candidate-sheet detection incl. multiple historical/hidden sheets, spillover-column exclusion, staffing/coverage-row exclusion, footer-note exclusion, blank-cell skip, multi-block accumulation), file-name/month-ambiguity resolution, a client-side mirror of the value-classification rules (FOLGA/FÉRIAS/explicit and extended-hours ranges/malformed ranges/MANHÃ-TARDE passthrough — UX-only, the RPC is still authoritative and independently re-derives everything), and the zero-diff publish-guard decision. 27 tests, all passing. The mapping/duplicate/diff/zero-diff-guard SQL logic itself was verified manually against the live RPC with synthetic far-future-month data (never touching real schedule rows), following this project's established "typecheck/lint/build + manual verification" convention rather than adding live-database integration tests.

**QA (product owner): PASS**, including: real `.xlsx` upload; September filename/month auto-selection; the ambiguous-filename/multiple-sheets selection prompt; canonical `escala_nome_planilha` employee mapping; an unmapped employee with real schedule data correctly blocking; zero-diff revision correctly refusing to offer/create a redundant publication; a real one-cell revision (Favacho, 18/09, A confirmar → 16:00–22:00 — an intentional operational correction) producing an understandable diff and publishing successfully as `v3`; Histórico correctly showing `v1`/`v2` as substituída and `v3` as ativa with the right row counts; employee-facing Escala (Dia/Semana/Mês) and Gestão privacy unaffected. Mobile QA of the uploader itself was explicitly out of scope for this closeout (primarily a desktop administrative workflow); reasonable responsive behavior is maintained but was not specifically optimized for mobile.

**Carried FUTURE / OPEN items (not blockers, not part of this milestone):** the `motivo da folga` classification (folga reason/type — still concept-only, per Milestone 4C's own carried item); full schedule authoring inside Portal; individual shift/cell patching; employee-initiated change requests or shift swaps; automated publish notifications (a "Copiar mensagem para WhatsApp" convenience remains a nice-to-have, not built); rollback UI; sophisticated Escala analytics; a Brazilian labor-law compliance engine; Admin holiday-management UI.
...
### Milestone 4D.1 — Contagem de Embalagens: resumable draft

**Status:** IMPLEMENTED / QA COMPLETE

V1's counting form was single-shot: nothing was persisted until the final submit, so an unfinished count existed only in the browser's component state and was lost on a closed tab or a lost connection. 4D.1 adds a resumable draft on top of the same submission/review model — it does **not** redesign or replace anything from Milestone 4D above.

**One active count, store-wide.** At most one packaging count can be in progress at a time — enforced by Postgres itself via a partial unique index on `contagens.status` (`where status = 'em_andamento'`), not by application logic. Any authenticated employee opening **Nova contagem** resumes that same draft rather than starting a competing one; if none exists, one is created for them. This matches Portal's single-store model (no `loja_id`/store-scoping concept exists anywhere in the schema).

**Who started it vs. who finalized it.** `contagens` gained `iniciado_por` / `iniciado_em` ("who started the draft and when"), shown to whoever opens the form as "Contagem iniciada por {nome} em {data/hora}." These are distinct from `submetido_por` / `submetido_em` ("who finalized it and when"), which are only set at the moment **Finalizar contagem** succeeds — the two columns are usually different employees, since resuming and finalizing are open to anyone. `submetido_por`/`submetido_em` are consequently nullable now (never null once `status` is `pendente_revisao` or `revisada`; always null while `em_andamento` — enforced by an updated `contagens_check` constraint alongside the existing `revisada_por`/`revisada_em` pairing).

**Explicit Salvar, not autosave.** An automatic debounce-plus-blur autosave was built and QA'd through two rounds of fixes, but kept failing in practice (a steady field-to-field data-entry flow rarely leaves any single field idle long enough for a debounce to fire) and was dropped per explicit product-owner direction in favor of a plain **Salvar** button — simpler to reason about, and there is no debounce-timing or effect-churn class of bug left to have. Salvar sends whatever is currently filled in (no completeness requirement — that stays a Finalizar-only rule) via `salvar_progresso_contagem`, shows "Salvando..." while in flight and "Progresso salvo" after, uses the same primary-blue treatment as Finalizar, and is `position: sticky` at the top of the form so it stays visible while the item cards scroll underneath it.

**Cancelar contagem.** A ghost-styled action below Finalizar opens a destructive-confirm `AlertDialog` (matching the existing Fechamento unsaved-data-confirm pattern: the safe "Continuar contagem" choice styled primary, the destructive "Sim, cancelar contagem" choice styled outlined) that abandons the draft outright via `cancelar_contagem_ativa`. Any authenticated employee can cancel, matching resume/finalize's "any authorized employee" model. The RPC's own `delete ... where status = 'em_andamento'` guarantees a `pendente_revisao`/`revisada` count can never be reached this way — abandoning a draft is not a way to erase a finalized submission.

**Schema.** Three additive migrations on top of `20260827_001`/`20260827_002` (unchanged): `20260912_001_add_contagem_draft_resume.sql` (adds `iniciado_por`/`iniciado_em` with backfill from the pre-existing `submetido_por`/`submetido_em` for historical rows, relaxes `submetido_por`/`submetido_em` to nullable, extends the `status` check to include `em_andamento`, rewrites `contagens_check`, adds the `contagens_uma_ativa_idx` partial unique index, and adds `get_or_start_contagem_ativa` + `salvar_progresso_contagem`); `20260912_002_fix_contagens_submetido_em_default.sql` (a same-day fix caught during post-apply verification, before any real draft existed: `submetido_em`'s leftover V1 `default now()` was still firing on a bare `em_andamento` insert, immediately tripping the new `contagens_check`); `20260912_003_add_cancelar_contagem_ativa.sql`. All applied via Supabase MCP `apply_migration`, immutable thereafter.

**RPCs (all SECURITY DEFINER, `set search_path = public`, session-authenticated — same hardening convention as every other RPC in this codebase).** `submeter_contagem` is retired and replaced by `finalizar_contagem(p_session_token, p_id_contagem, p_itens jsonb, p_observacao text)` — identical completeness validation to V1's `submeter_contagem`, but now `UPDATE`s the caller's already-open `em_andamento` row (stamping `submetido_por`/`submetido_em`, flipping status to `pendente_revisao`) instead of `INSERT`ing a fresh one; a `for update` + status re-check in the `UPDATE`'s `WHERE` clause make a double-finalize safe, mirroring `marcar_contagem_revisada`'s existing guard. New: `get_or_start_contagem_ativa(p_session_token)` (any employee; `pg_advisory_xact_lock` serializes concurrent "start" attempts the same way this codebase already does for `lista_vez`), `salvar_progresso_contagem(p_session_token, p_id_contagem, p_itens jsonb)` (any employee; upserts partial item values, no completeness check, rejects a non-`em_andamento` target), `cancelar_contagem_ativa(p_session_token, p_id_contagem)` (any employee; deletes only while still `em_andamento`). `get_contagem_catalogo`, `get_contagens_pendentes`, `get_contagem_historico`, `get_contagem_detalhe`, and `marcar_contagem_revisada` are all untouched — none of them ever see an `em_andamento` row.

**Verified live against the database** (throwaway sessions minted via `issue_employee_session`, revoked after, no residual test data): a draft started by one employee was correctly resumed — with its autosaved item values — by a different employee's session; `finalizar_contagem` correctly rejected an incomplete payload and correctly recorded a `submetido_por` different from `iniciado_por` on success; `cancelar_contagem_ativa` correctly refused to touch an already-`revisada` row and correctly deleted a real `em_andamento` draft (with product-owner sign-off, since it was the owner's own in-progress QA draft at the time) when invoked by an employee other than the one who started it.

**Bug found and fixed during QA: stale draft after navigating away and back.** The active-draft query had no cache expiry, so React Query kept a previous visit's field values cached after the form unmounted; returning to the form rendered that stale snapshot immediately (before any network call), and the "seed the form once per draft id" guard then refused to re-apply the correct values once the background refetch actually completed — Salvar's own success was never in question, only the client's redisplay of it. Fixed by setting `gcTime: 0` on that query, forcing a genuine fetch (and a real loading state) every time the form is reopened rather than a stale render followed by a silently-discarded correction.

**Frontend.** New hooks `useContagemAtiva` (fetch-or-start the active draft), `useSalvarProgressoContagem`, `useCancelarContagem`; `useFinalizarContagem` replaces `useSubmeterContagem`. `NovaContagemView` seeds its fields from the active draft's last-saved values once per draft identity, then behaves as a normal controlled form until Salvar, Finalizar, or Cancelar.

**FUTURE — management correction/reopen.** Reopening or correcting an already-`pendente_revisao`/`revisada` count is explicitly out of scope for 4D.1, per product-owner direction — this remains future work, not started, not scaffolded.

Validated: `npm run typecheck` (clean), `npm run lint` (0 errors; the same 6 pre-existing `react-refresh` warnings in `src/components/ui/*`, no new ones), `npm run build` (clean) after every change in this milestone. **Browser QA (product owner): PASS**, including the resumable-draft flow, the Salvar/Cancelar actions, the sticky Salvar bar, and the stale-draft fix above.
...
### Milestone 4E — Consulta de Estoque — Inventory Backend V1

**Status:** BACKEND IMPLEMENTED, REAL SYNC DONE & VALIDATED (corrected applicable-grade snapshot, 2026-09-10). Employee UI is the next milestone.

Bounded backend milestone for Consulta de Estoque (section 9). No Consulta frontend, no barcode work.

Scope delivered:

- **Migrations** (tracked, additive, applied to prod): `20260909_001` replaces the obsolete, untracked, empty barcode-centric `estoque_snapshot` prototype with the locked schema — `estoque_sync_execucoes`, `estoque_snapshot` (unique `sync_id, produto, cor_codigo, tamanho_key`), `estoque_cores_mapeamento` (unique `cor_codigo, cor_descricao_linx`); all RLS-enabled, zero policies. `20260909_002` seeds 419 curated colour mappings from `cores_portal_final.csv` (idempotent, reproducible for a future separate dev/prod environment). `20260909_003` adds the read RPCs. `20260909_004` filters `get_produto_estoque_detalhe` to labelled grade positions (defensive — the corrected extraction already stores only those). `20260909_005` grants `service_role` the minimal DML (`insert`/`select` on the snapshot tables, `+update` on the execution ledger) the on-prem sync needs — this project does not carry the blanket Supabase table grants; it does not add a policy, weaken RLS, or expose anything to `anon` / `authenticated` / the frontend.
- **Read RPCs** (SECURITY DEFINER, session-validated, `anon`-granted; read only the latest `status = 'sucesso'` sync): `buscar_produtos_estoque` (prefix on `produto`, min 2 chars, secondary `desc_produto` contains), `get_produto_estoque_detalhe` (exact `produto` → all colours + full dynamic size grade + `sync_concluido_em`; `cor_descricao_linx` never returned; unmapped colour → `cor_nome_portal = null`), `get_estoque_freshness`.
- **Manual sync** — `scripts/sync-estoque/` (Node ESM, run by hand from the on-prem PC; `--dry-run` supported). Connects to Linx `aplserver` / `Lacoste_60420` (dedicated read-only SQL login `portal_benvisi_estoque`), runs the canonical `CROSS APPLY VALUES` extraction — reads all 48 `ESTOQUE_PRODUTOS` `ES*` positions per qualifying `(produto, cor)`, `JOIN`s the dynamic size labels from `PRODUTOS_TAMANHOS` via `PRODUTOS.GRADE` so **only labelled grade positions survive** (`PRODUTO_CORES` for the colour code/description; `PRODUTOS_BARRA` not joined), filter `FILIAL = 'LACOSTE SHOPPING  MANAUS' AND ESTOQUE > 0`. Opens an `executando` row, validates locally (count, canonical-key uniqueness, `tamanho_venda` non-blank on every row, integer quantities, **stops** on any size-level negative), bulk-publishes (1 000-row chunks, `service_role`) tagged with the new `sync_id`, verifies the Supabase count, marks `sucesso`; any failure marks `erro` and leaves the previous successful snapshot intact.

**Snapshot grain (locked).** One row per qualifying **produto + cor + applicable labelled size position** (`sync_id, produto, cor_codigo, tamanho_key`). Linx physically stores up to 48 positions (`ES1..48` / `TAMANHO_1..48`); the extraction reads all 48 and keeps a position only when its `PRODUTOS_TAMANHOS` label is non-null/non-blank — the real grade, including its zero-stock sizes. Unused/unlabelled padding positions are discarded; a 2026-09 diagnostic confirmed all 50 302 padding rows in the raw fixed-48 rectangle carry `quantidade_estoque = 0` (0 positive, 0 negative), so nothing is lost. `tamanho_key` is retained purely for deterministic ordering and is never shown to employees; the UI renders `tamanho_venda`. No hard-coded Portal maximum grade size — 30 grades in use, 323 valid positions, highest currently-used `tamanho_key = 20`, positions 21–48 flow through unchanged if a future grade needs them.

**Real syncs.** First run `sync_id a9402199-54a4-4392-abdb-5d132aa5752d` (2026-09-09) published the raw fixed-48 rectangle (65 808 rows) before the grain was corrected — retained as history, no longer visible. Corrected run **`sync_id e009b5aa-d1f5-4d01-a5b7-0c782cd27a06`** (2026-09-10): 15 506 rows extracted = 15 506 published, `status = sucesso`, and now authoritative via `estoque_sync_atual()`. Reconciliation vs the 2026-09-09 applicable-grade inspection (drift-tolerant): produto_cores 1 371 = 1 371; applicable rows ≈ 15 506; sizes > 0 3 705; applicable sizes = 0 11 801; sizes < 0 **0**; 578 distinct `produto`.

**Validation** (corrected snapshot): latest execution `sucesso`, `linhas_extraidas = linhas_publicadas = 15 506`, Supabase row count matches; 0 duplicate `(sync_id, produto, cor_codigo, tamanho_key)`; **0 rows with a null/blank `tamanho_venda`** (no padding in the visible snapshot); every qualifying produto/cor represented; **colour-map coverage 100 %** (419 distinct `(cor_codigo, cor_descricao_linx)` pairs, all 419 mapped, 0 unmapped); prefix search `L1212` → only `L1212-21/23`, `PH` → 50 hits all `PH*`; `get_produto_estoque_detalhe` returns every colour with its labelled grade ordered by `tamanho_key` (zeros retained), `cor_nome_portal`/`cor_familia` populated, no `cor_descricao_linx`, single `sync_concluido_em`; a newer `erro`/`executando` execution does not change visibility or freshness; invalid session raises `INVALID_SESSION`. `npm run typecheck`, `npm run lint`, `npm run build` all pass.
...
## ADR-022 — Inventory Freshness Is the Latest Successful Sync, Never a Partial One

**Status:** APPROVED — implemented in Milestone 4E

Portal inventory (`estoque_atual`) is a periodically-refreshed read model of Linx, not a live view. Each refresh is one `estoque_sync_execucoes` row (`executando` → `sucesso` | `erro`).

The employee-visible snapshot is defined **only** by the most recent execution with `status = 'sucesso'` and a non-null `concluido_em`. An in-progress or failed run is never visible, and a failed run never invalidates the previous successful snapshot — its rows simply stay attached to the older `sync_id`. Prior successful snapshots are retained for now (no aggressive cleanup).

Employee-facing colour identity is always `cor_codigo + cor_nome_portal` from the curated `estoque_cores_mapeamento` dictionary. The Linx source description (`cor_descricao_linx`) is join metadata only and is never returned by a read RPC; an unmapped colour returns `cor_nome_portal = null` rather than falling back to the source description.

---

## ADR-023 — Snapshot Grain Is Applicable/Labelled Grade Positions, Not the Linx 48-Slot Rectangle

**Status:** APPROVED — implemented in Milestone 4E (2026-09-10 correction)

Linx physically stores each product/colour's stock across up to 48 fixed columns (`ESTOQUE_PRODUTOS.ES1..ES48`), with the size labels for a grade in `PRODUTOS_TAMANHOS.TAMANHO_1..48`. Most grades use far fewer than 48 positions.

The canonical `estoque_atual` grain is **one row per qualifying `produto` + `cor_codigo` + applicable/labelled size position** — every real size in the product's grade, including applicable sizes whose quantity is `0`. Unused positions (those whose `TAMANHO_<n>` is null/blank for that grade) are **not** persisted, even though Linx physically has a column for them. A 2026-09 diagnostic confirmed every unlabelled padding position across current inventory is quantity `0` (0 positive, 0 negative), so discarding them loses no stock information.

The extraction still reads all 48 physical positions and discards a position only for being unlabelled — never by a `tamanho_key` threshold. There is no hard-coded Portal maximum grade size; a future grade using positions 21–48 flows through unchanged.

`tamanho_key` (1..48, the source column index) is retained solely as deterministic grade-ordering metadata. It is never an employee-facing value — the UI renders only the `tamanho_venda` labels applicable to that product's grade, ordered by `tamanho_key`.
...
The Milestone 4E predecessor read model: one full new ~15k-row snapshot inserted per sync run, identified by `sync_id`. Superseded by `estoque_atual`; dropped 2026-09-11 (`chore/estoque-v1-cleanup`) along with `estoque_sync_atual()` after a live dependency audit confirmed neither was referenced by any view, trigger, or function.
...
Curated colour dictionary keyed by `(cor_codigo, cor_descricao_linx)`, mapping the Linx source colour to the employee-facing `cor_nome_portal` (shown as `cor_codigo · cor_nome_portal`) plus `cor_familia`. `cor_descricao_linx` is internal and is never shown to employees.
