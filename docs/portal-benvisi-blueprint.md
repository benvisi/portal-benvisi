...
- Level 2 (navigation tiles inside a section hub, e.g. Operações, Conhecimento & Cultura) uses a muted sage/gray-green (`#708A7D`), with white text/icons. Measured contrast against white is ~3.74:1 — still short of the formal WCAG AA text threshold (4.5:1), but higher than the color it replaced (`#2F9E67`, ~3.24:1) and above the 3:1 AA threshold for large text/UI components. The app's dark text color only reaches ~3.92:1 against this background — not enough of a gain to justify losing the white-text look Level 2 shares with Level 1 — so white text/icons are kept;
- Level 3 is reserved for a still-deeper future navigation tier — no such tier exists in the app today. It is now an independent, still lighter/more muted sage-gray (`#CFD8D3`), no longer reusing the table zebra-stripe color, paired with dark (not white) text/icons, since that background is light. No Level 4 exists yet; white would be the natural next step in this progression if a fourth tier is ever needed;
